...
pub fn runBenchmarks(allocator: Allocator, benchmarks: []const Spec, options: Options) !bool {
    ...
    for (benchmarks) |entry| {
        if (options.filter) |filter| {
            if (!matchesFilter(filter, entry.name)) continue;
        }
...
fn matchesFilter(filter: []const u8, name: []const u8) bool {
    var pattern = filter;
    const anchor_start = mem.startsWith(u8, pattern, "^");
    if (anchor_start) pattern = pattern[1..];

    const anchor_end = mem.endsWith(u8, pattern, "$");
    if (anchor_end) pattern = pattern[0 .. pattern.len - 1];

    if (!mem.containsAtLeast(u8, pattern, 1, "*")) {
        if (anchor_start and anchor_end) return mem.eql(u8, name, pattern);
        if (anchor_start) return mem.startsWith(u8, name, pattern);
        if (anchor_end) return mem.endsWith(u8, name, pattern);
        return mem.containsAtLeast(u8, name, 1, pattern);
    }

    if (anchor_start and anchor_end) return globMatch(pattern, name);
    if (anchor_start) {
        var end: usize = 0;
        while (end <= name.len) : (end += 1) {
            if (globMatch(pattern, name[0..end])) return true;
        }
        return false;
    }
    if (anchor_end) {
        ...
        while (start <= name.len) : (start += 1) {
            if (globMatch(pattern, name[start..])) return true;
        }
        return false;
    }
    ...
    while (start <= name.len) : (start += 1) {
        var end = start;
        while (end <= name.len) : (end += 1) {
            if (globMatch(pattern, name[start..end])) return true;
        }
    }
    return false;
}
...
test "filter matches substrings anchors and globs" {
    try testing.expect(matchesFilter("Alloc", "BenchmarkAllocPerLine"));
    try testing.expect(!matchesFilter("Table", "BenchmarkAllocPerLine"));

    try testing.expect(matchesFilter("^BenchmarkAscii", "BenchmarkAsciiCount/Table"));
    try testing.expect(!matchesFilter("^Ascii", "BenchmarkAsciiCount/Table"));

    try testing.expect(matchesFilter("Table$", "BenchmarkAsciiCount/Table"));
    try testing.expect(!matchesFilter("Table$", "BenchmarkAsciiCount/TableLarge"));

    try testing.expect(matchesFilter("^BenchmarkAsciiCount/Table$", "BenchmarkAsciiCount/Table"));
    try testing.expect(!matchesFilter("^BenchmarkAsciiCount/Table$", "BenchmarkAsciiCount/TableLarge"));

    try testing.expect(matchesFilter("Ascii*Table", "BenchmarkAsciiCount/Table"));
    try testing.expect(matchesFilter("^Benchmark*/*Table$", "BenchmarkAsciiCount/Table"));
    try testing.expect(!matchesFilter("^Benchmark*/*Scalar$", "BenchmarkAsciiCount/Table"));
}
